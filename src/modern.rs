...
use crate::{to_pwstr, shellbag::{ShellBagList, WindowBagInfo, ShellFolderItem, ShellItem, ShellVolumeItem, ShellFileItem, ShellBagPath, ShellBags, ShellNetworkItem, NodeSlot}, read_reg_u32_value, read_reg_sz_value, enumerate_keys, read_reg_bin_value};
...
pub unsafe fn get_mru_list(bag_mru_key : HKEY) -> Result<Vec<u32>,WIN32_ERROR> {
    ...
    let readed = RegQueryValueExW(bag_mru_key, PCWSTR(mru_list_str.as_ptr()),reserved as _, &mut data_type,readed_data.as_mut_ptr(), &mut capacity);
    if readed.is_err() {
        return Err(readed);
    }
...
#[cfg(test)]
mod test_shellbag {
    use super::{read_shell_bags_modern};

    #[test]
    fn test_enumerate_bags() {
        unsafe {
            /* 
            let shell_key = get_shell_key().expect("Must exists Shell key");
            let bags_key = get_bags_key(shell_key).expect("Must exists Bags key");
            let bags = read_all_bags(bags_key).expect("Must get all bags");
            for (slot, bag) in bags.node_slots {
                for (window_name, info) in bag {
                    println!("{}[{}]={}({}) vid={}", slot,window_name, info.slot,info.uuid, info.vid);
                }
            }*/
            let list = read_shell_bags_modern().expect("Must return all shellbags");
            println!("{:?}", list.usr_class.list);
            println!("{:?}", list.ntuser.list);
            // 
        }
    }
}